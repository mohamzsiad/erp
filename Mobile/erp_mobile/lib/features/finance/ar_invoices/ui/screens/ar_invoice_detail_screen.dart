...
class ArInvoiceDetailScreen extends ConsumerWidget {
  const ArInvoiceDetailScreen({super.key, required this.id});
  final String id;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(arInvoiceDetailProvider(id));
    return Scaffold(
      backgroundColor: AppColours.background,
      appBar: AppBar(
        title: const Text('AR Invoice'),
        backgroundColor: AppColours.primary,
        foregroundColor: AppColours.surface,
        actions: [
          IconButton(
            icon: const Icon(Icons.refresh),
            onPressed: () => ref.invalidate(arInvoiceDetailProvider(id)),
          ),
        ],
      ),
      body: async.when(
        ...
        error: (e, _) => ErrorState(
          message: e.toString(),
          onRetry: () => ref.invalidate(arInvoiceDetailProvider(id)),
        ),
        data: (inv) => _ArInvoiceBody(inv: inv),
      ),
    );
  }
}

class _ArInvoiceBody extends ConsumerWidget {
  const _ArInvoiceBody({required this.inv});
  final ArInvoice inv;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final canApprove = ref.read(authProvider.notifier)
        .hasPermission('FINANCE', 'AR_INVOICE', 'APPROVE');
    final canPost = ref.read(authProvider.notifier)
        .hasPermission('FINANCE', 'AR_INVOICE', 'POST');
      ...
      children: [
        _Card(child: Column(
          ...
          children: [
            ...
            _InfoRow('Currency', '${inv.currencyCode} (Rate: ${inv.exchangeRate})'),
            if (inv.projectName != null) _InfoRow('Project', inv.projectName!),
            if (inv.paymentTerms != null) _InfoRow('Payment Terms', inv.paymentTerms!),
            ...
            if (inv.receivedAmount > 0) _TotalRow('Received', inv.receivedAmount),
            if (inv.balanceDue > 0)
              _TotalRow('Balance Due', inv.balanceDue, bold: true,
                  color: isOverdue ? AppColours.statusRejected : AppColours.statusPartial),
          ],
        )),
                  ...
                  children: [
                    Text(l.glAccountCode,
                        style: const TextStyle(
                            fontFamily: 'monospace', color: AppColours.primary,
                            fontSize: 11, fontWeight: FontWeight.bold)),
        ...
        if (inv.status == 'DRAFT' && canApprove)
          _ActBtn('Approve Invoice', Icons.check_circle_outline,
              AppColours.statusApproved, () => _action(context, ref, 'approve')),
        if (inv.status == 'APPROVED' && canPost) ...[
          const SizedBox(height: 8),
          _ActBtn('Post Invoice', Icons.post_add_outlined,
              AppColours.statusSubmitted, () => _action(context, ref, 'post')),
        ],
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text(label, style: TextStyle(
              ...
              fontSize: bold ? 15 : 13)),
          AmountText(amount, style: TextStyle(
              fontWeight: bold ? FontWeight.bold : FontWeight.w500,
              fontSize: bold ? 15 : 13, color: color)),
        ]),
      );
}
