...
import { Download, RefreshCw, Loader2, ChevronRight, X } from 'lucide-react';
import { clsx } from 'clsx';
import { AgGridReact } from 'ag-grid-react';
import type { ColDef, ICellRendererParams } from 'ag-grid-community';
import 'ag-grid-community/styles/ag-grid.css';
import 'ag-grid-community/styles/ag-theme-alpine.css';
import { useArAging } from '../../../api/finance';
import { downloadExcel } from '../../../utils/excelExport';

interface AgingInvoice {
  id: string; docNo: string; invoiceDate: string; dueDate: string;
...
interface AgingRow {
  ...
  current: number; days0_30: number; days31_60: number; days61_90: number; over90: number; total: number;
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200">
          <div>
            <h3 className="font-bold text-[#1F4E79] text-lg">{row.customerName}</h3>
            ...
            <tbody>
              {row.invoices.map(inv => (
                <tr key={inv.id} className={clsx('border-b border-gray-100 hover:bg-gray-50',
                  inv.daysOverdue>90&&'bg-red-50', inv.daysOverdue>60&&inv.daysOverdue<=90&&'bg-orange-50',
                  ...
                  <td className={clsx('px-3 py-1.5 text-right font-semibold',
                    inv.daysOverdue<=0?'text-green-600':inv.daysOverdue<=30?'text-yellow-600':inv.daysOverdue<=60?'text-orange-600':'text-red-600')}>
...
export default function CustomerAgingPage() {
  ...
  const { data, isFetching } = useArAging({ asAt: runParams?.asAt }, !!runParams);

  const rows = useMemo<AgingRow[]>(() => data?.lines ?? [], [data]);
  const grandTotals = useMemo(() => {
    if (!rows.length) return null;
    return rows.reduce((a,r) => ({
      current:   a.current+r.current, days0_30:a.days0_30+r.days0_30,
      days31_60: a.days31_60+r.days31_60, days61_90:a.days61_90+r.days61_90,
      over90:    a.over90+r.over90, total:a.total+r.total,
    }), {current:0,days0_30:0,days31_60:0,days61_90:0,over90:0,total:0});
  }, [rows]);

  const colDefs = useMemo<ColDef<AgingRow>[]>(() => [
    {
      headerName:'Customer', field:'customerName', flex:3, minWidth:200, pinned:'left',
      cellRenderer:(p:ICellRendererParams<AgingRow>) => {
        const r=p.data!;
        return <button onClick={()=>setDrillRow(r)} className="flex items-center gap-1 text-[#1F4E79] hover:underline font-medium text-xs w-full text-left">
          <ChevronRight size={12} className="shrink-0"/>
          <span className="text-gray-400 font-mono mr-1">{r.customerCode}</span>{r.customerName}
        </button>;
      },
    },
    { headerName:'Current',    field:'current',   width:130, type:'numericColumn', cellStyle:{fontFamily:'monospace',fontSize:'12px',color:'#166534',textAlign:'right'}, valueFormatter:(p)=>fmt3(p.value) },
    { headerName:'1–30 Days',  field:'days0_30',  width:130, type:'numericColumn', cellStyle:{fontFamily:'monospace',fontSize:'12px',textAlign:'right',color:'#854d0e'}, valueFormatter:(p)=>fmt3(p.value) },
    { headerName:'31–60 Days', field:'days31_60', width:130, type:'numericColumn', valueFormatter:(p)=>fmt3(p.value),
      cellStyle:(p)=>({fontFamily:'monospace',fontSize:'12px',textAlign:'right',backgroundColor:p.value>0?'#fef9c3':undefined,color:p.value>0?'#713f12':'#374151',fontWeight:p.value>0?'600':'400'}) },
    { headerName:'61–90 Days', field:'days61_90', width:130, type:'numericColumn', valueFormatter:(p)=>fmt3(p.value),
      cellStyle:(p)=>({fontFamily:'monospace',fontSize:'12px',textAlign:'right',backgroundColor:p.value>0?'#fed7aa':undefined,color:p.value>0?'#7c2d12':'#374151',fontWeight:p.value>0?'600':'400'}) },
    { headerName:'> 90 Days',  field:'over90',    width:130, type:'numericColumn', valueFormatter:(p)=>fmt3(p.value),
      cellStyle:(p)=>({fontFamily:'monospace',fontSize:'12px',textAlign:'right',backgroundColor:p.value>0?'#fee2e2':undefined,color:p.value>0?'#7f1d1d':'#374151',fontWeight:p.value>0?'700':'400'}) },
    { headerName:'Total', field:'total', width:150, type:'numericColumn', pinned:'right',
      cellStyle:{fontFamily:'monospace',fontSize:'12px',textAlign:'right',fontWeight:'700',color:'#1F4E79'}, valueFormatter:(p)=>fmt3(p.value) },
  ], []);

  const handleExport = () => {
    if (!rows.length) return;
    const allRows = [...rows.map(r => ({ cells:[r.customerCode,r.customerName,r.current||null,r.days0_30||null,r.days31_60||null,r.days61_90||null,r.over90||null,r.total||null], bold:false, isCurrency:[false,false,true,true,true,true,true,true] }))];
    ...
    downloadExcel([{ sheetName:`AR Aging ${runParams?.asAt??''}`, headers:['Code','Customer','Current','1-30 Days','31-60 Days','61-90 Days','>90 Days','Total'], rows:allRows, currencyColIndices:[2,3,4,5,6,7], colWidths:[10,35,16,16,16,16,16,18] }], `AR_Aging_${runParams?.asAt??today}`);
  };
  ...
  return (
    <div className="flex flex-col h-full min-h-0 p-4 gap-3">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#1F4E79]">Customer Aging</h1>
          <p className="text-sm text-gray-500 mt-0.5">Accounts Receivable — Outstanding Invoice Aging</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-3 flex items-end gap-4">
        ...
        <span className="text-xs text-gray-400 self-center">Click a customer row to see invoice detail</span>
      </div>

      <div className="flex items-center gap-4 text-xs text-gray-600">
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-green-200 inline-block"/> Current</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-yellow-100 border border-yellow-300 inline-block"/> 31–60 days</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-orange-200 inline-block"/> 61–90 days</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-red-200 inline-block"/> &gt;90 days</span>
      </div>
      ...
      <div className="flex-1 min-h-0 ag-theme-alpine rounded-lg overflow-hidden border border-gray-200">
        {!runParams ? (
          ...
          <AgGridReact<AgingRow> ref={gridRef} rowData={rows} columnDefs={colDefs}
            suppressCellFocus suppressRowClickSelection animateRows={false} rowBuffer={20} rowHeight={32}
      ...
      {grandTotals && (
        <div className="bg-[#1F4E79] text-white rounded-lg px-4 py-2.5 grid grid-cols-7 text-xs font-semibold">
          <span className="col-span-1">TOTALS ({rows.length} customers)</span>
